  ...
  # rho_grid_search: grid search candidates of tuning parameter for selecting number of ranks in each subnetwork's decomposition. 
{
  ...
  # preprocess Y to speed up 
  ## Yraw = Y
  Y_demean = sweep(Y,2,apply(Y,2,mean),"-") 
  Y_preprocess = Locus_preprocess(Y_demean, q)
  ...
  for(rho in rho_grid_search)
  {
    for(phi in phi_grid_search)
    {
      ...
      LOCUS_output = LOCUS(Y_preprocess, q=q, V=V, MaxIteration=MaxIteration, penalty=penalty, phi=phi, approximation=T, 
                           preprocess=F, espli1=espli1, espli2=espli1, rho=rho, silent=T, demean=F)
      bic_value = calculate_bic(Y_preprocess, LOCUS_output$A, LOCUS_output$S)
